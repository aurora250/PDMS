 ...
 *   <li><b>居住证管理</b> —— 申请、审批、签发、续期居住证，签发时自动关联登记记录。
 *   <li><b>常住人口管理</b> —— 居住登记、信息变更（仅更新非空字段）、注销。
...
public class FloatingPopulationServiceImpl implements FloatingPopulationService {
     ...
     * 流动人口登记。
     *
     * <p>登记日期自动设为当天，居住证编号初始为 {@code null}，待签发时回填。
     *
     ...
     * @return 更新后的登记记录
     * @throws BusinessException 登记记录不存在时抛出
     */
    @Override
    @Transactional
     ...
     * 申请居住证。
     *
     * <p>自动以 {@code "RP"} 为前缀生成 20 位唯一证件编号，签发日期默认为当天， 有效期默认为签发日期起一年，初始状态为"有效"。
     *
     * @param permit 居住证实体
     * @return 创建后的居住证实体
     */
     ...
     * @return 审批后的居住证实体
     * @throws BusinessException 居住证不存在时抛出
     */
    @Override
    @Transactional
     ...
     * 签发居住证。
     *
     * <p>设置签发日期和有效期（签发日期起一年），并将证件编号回填到关联的流动人口登记记录中。
     *
     * @param id 居住证 ID
     * @return 签发后的居住证实体
     * @throws BusinessException 居住证不存在时抛出
     ...
     * 续期居住证。
     *
     * <p>仅状态为"有效"的居住证方可续期，续期后有效期延长一年。 续期记录保存原有效期和新有效期信息。
     *
     * @param id 居住证 ID
     * @param renewal 续期信息
     * @return 续期记录
     * @throws BusinessException 居住证不存在或已过期时抛出
     */
    @Override
    @Transactional
     ...
     * 变更常住人口居住信息。
     *
     * <p>仅更新请求中提供的非空字段，未提供的字段保持不变， 避免覆盖已有有效数据。
     *
     * @param rid 记录 ID
     ...
     * 获取人口热力图数据。
     *
     * <p>基于常住人口分布数据生成区域维度的热力图数据， 包含区域 ID、地址类型和居民 UUID。
     *
     * @return 热力图数据列表
     */
    @Override
     ...
     * 获取人口趋势数据。
     *
     * <p>基于流动人口登记记录生成时间维度的趋势分析数据， 包含登记日期和记录 ID。
     *
     * @return 趋势数据列表
     */
    @Override
