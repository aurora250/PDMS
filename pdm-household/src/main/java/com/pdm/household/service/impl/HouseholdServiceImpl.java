 ...
 * <li><b>业务审批</b> —— 四级审批流（采集员录入 → 街道办初审 → 民警复核 → 市局审批）， 覆盖登记、注销、户主变更等业务。</li>
...
public class HouseholdServiceImpl implements HouseholdService {
     ...
     *
     * <p>
     * 若未指定户口本编号，则自动以 {@code "HB"} 为前缀生成 20 位唯一编号。 新户口本建立日期为当天，初始状态为"审批中"。
     *
     * @param book
     *            户口本实体
     ...
     *
     * <p>
     * 根据户口本编号查找原记录，校验存在后返回。生产环境应生成新编号并复制原数据。
     *
     * @param bookNo
     *            户口本编号
     * @return 原户口本实体
     ...
     *
     * <p>
     * 逻辑与补办相同，直接复用 {@link #reissueBook(String)}。
     *
     * @param bookNo
     *            户口本编号
     * @return 原户口本实体
     ...
     *
     * <p>
     * 适用于登记、注销、户主变更等业务。提交后进入四级审批流程： <b>采集员录入 → 街道办初审 → 民警复核 → 市局审批</b>。
     * 初始状态为"审批中"，处理日期为当天。
     *
     * @param request
     *            业务申请实体
     ...
     *
     * <p>
     * 根据申请 ID 查找记录，更新审批状态、处理人和驳回原因（可选）。
     *
     * @param rid
     *            业务申请 ID
     * @param status
     ...
     *
     * <p>
     * 根据申请人 UUID 查询其所有迁移记录，按创建时间倒序排列。
     *
     * @param residentUuid
     *            居民 UUID
     ...
     *
     * <p>
     * 若未指定证件编号，则自动以 {@code "AP"} 为前缀生成 20 位唯一编号。 签发日期为当天，初始状态为"有效"。
     *
     * @param permit
     *            准迁证实體
     ...
     *
     * <p>
     * 若未指定证件编号，则自动以 {@code "MP"} 为前缀生成 20 位唯一编号。 签发日期为当天，初始状态为"有效"。
     *
     * @param permit
     *            迁移证实体
