 ...
 *
 * <ul>
 *   <li><b>户口本管理</b> —— 申请、补办、换新户口本。
 *   <li><b>业务审批</b> —— 四级审批流（采集员录入 → 街道办初审 → 民警复核 → 市局审批）， 覆盖登记、注销、户主变更等业务。
 *   <li><b>迁移管理</b> —— 提交迁移申请、审批迁移、查询迁移轨迹。
 *   <li><b>证件签发</b> —— 签发准迁证和迁移证。
 *   <li><b>区域查询</b> —— 获取省级区域树和按父级查询子区域。
 * </ul>
 *
 * @author freedom
 */
...
public class HouseholdServiceImpl implements HouseholdService {
     ...
     * 申请户口本。
     *
     * <p>若未指定户口本编号，则自动以 {@code "HB"} 为前缀生成 20 位唯一编号。 新户口本建立日期为当天，初始状态为"审批中"。
     *
     * @param book 户口本实体
     ...
     * 提交户政业务申请。
     *
     * <p>适用于登记、注销、户主变更等业务。提交后进入四级审批流程： <b>采集员录入 → 街道办初审 → 民警复核 → 市局审批</b>。 初始状态为"审批中"，处理日期为当天。
     *
    ...
    @Override
    @Transactional
    public HouseholdBusinessRequest approveBusiness(
            Long rid, String status, String handlerUuid, String rejectReason) {
    ...
    @Override
    @Transactional
    public HouseholdMigrationRequest approveMigration(
            Long rid, String status, String handlerUuid, String rejectReason) {
     ...
     * 签发准迁证。
     *
     * <p>若未指定证件编号，则自动以 {@code "AP"} 为前缀生成 20 位唯一编号。 签发日期为当天，初始状态为"有效"。
     *
     * @param permit 准迁证实體
     ...
     * 签发迁移证。
     *
     * <p>若未指定证件编号，则自动以 {@code "MP"} 为前缀生成 20 位唯一编号。 签发日期为当天，初始状态为"有效"。
     *
     * @param permit 迁移证实体
