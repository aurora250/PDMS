 ...
 * 户政管理控制器。
 *
 * <p>提供户政业务相关的 REST API 接口，包括户口本管理（申请、补办、换新）、 户政业务审批（登记、注销、户主变更）、户口迁移管理（申请、审批、轨迹查询）、
 * 证件签发（准迁证、迁移证）以及行政区划查询。 所有接口均需携带有效的 JWT 令牌方可访问。
 */
@RestController
...
public class HouseholdController {
     ...
     * 补办户口本。
     *
     * <p>根据户口本编号补办丢失或损坏的户口本。
     *
     * @param body 请求体，包含 {@code bookNo} 字段
     * @return 补办后的户口本实体
     */
     ...
     * 提交户政业务申请。
     *
     * <p>适用于户籍登记、注销、户主变更等业务场景。 提交后进入四级审批流程。
     *
     * @param req 业务申请实体
     * @return 创建后的业务申请实体
     */
    @PostMapping("/api/household/business")
    public Result<HouseholdBusinessRequest> submitBusiness(
            @RequestBody HouseholdBusinessRequest req) {
     ...
     * 审批户政业务。
     *
     * <p>处理人对待审批的户政业务进行审核，可审批通过或驳回。
     *
     * @param rid 业务申请 ID
     * @param body 请求体，包含 {@code status} 审批状态和 {@code rejectReason} 驳回原因
     * @param handlerUuid 处理人 UUID，从请求头 {@code X-User-Uuid} 中获取
     * @return 更新后的业务申请实体
     */
    @PutMapping("/api/household/business/{rid}/approve")
    public Result<HouseholdBusinessRequest> approveBusiness(
            @PathVariable Long rid,
            @RequestBody Map<String, String> body,
            @RequestHeader("X-User-Uuid") String handlerUuid) {
        return Result.success(
                householdService.approveBusiness(
                        rid, body.get("status"), handlerUuid, body.get("rejectReason")));
    }

    /**
     * 提交户口迁移申请。
     *
     * <p>居民提交户口迁移申请，包括迁入地和迁出地信息。
     *
     * @param req 迁移申请实体
     * @return 创建后的迁移申请实体
     */
    @PostMapping("/api/household/migration")
    public Result<HouseholdMigrationRequest> submitMigration(
            @RequestBody HouseholdMigrationRequest req) {
    ...
    public Result<HouseholdMigrationRequest> approveMigration(
            @PathVariable Long rid,
            @RequestBody Map<String, String> body,
            @RequestHeader("X-User-Uuid") String handlerUuid) {
        return Result.success(
                householdService.approveMigration(
                        rid, body.get("status"), handlerUuid, body.get("rejectReason")));
    }
     ...
     * 签发准迁证。
     *
     * <p>为通过迁移审批的居民签发准迁证，是户口迁移流程中的重要环节。
     *
     * @param permit 准迁证实體
     * @return 创建后的准迁证实体
     */
     ...
     * 获取行政区划区域树。
     *
     * <p>若提供 {@code parentId} 查询参数，则返回该父级下的子区域列表； 否则返回省级（顶层）区域列表。
     *
     * @param parentId 父级区域 ID（可选）
     * @return 区域列表
     */
    @GetMapping("/api/area")
    public Result<List<Area>> getAreaTree(@RequestParam(required = false) Long parentId) {
        if (parentId != null) return Result.success(householdService.getAreasByParent(parentId));
