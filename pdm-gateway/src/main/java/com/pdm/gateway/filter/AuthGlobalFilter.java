 ...
 *
 * <p>
 * 拦截所有经过网关的请求，对白名单以外的路径进行 JWT 令牌校验。 校验通过后从令牌中提取用户信息（UUID、用户名、角色）， 通过
 * {@code X-User-Uuid}、{@code X-Username}、{@code X-User-Role} 请求头传递给下游微服务。
 * 校验失败返回 401 未授权响应。
 * </p>
 *
 * <p>
 * 白名单路径：{@code /api/auth/login}、{@code /api/auth/register}、{@code /api/auth/refresh}。
 * 过滤器优先级为 -100（高优先级），确保在路由之前执行。
 * </p>
 */
@Slf4j
@Component
...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
     ...
     *
     * <p>
     * 白名单路径直接放行；其他路径须携带有效的 Bearer Token， 校验通过后将用户信息写入请求头传递给下游服务。
     * </p>
     *
     * @param exchange
     *            当前请求-响应交换对象
