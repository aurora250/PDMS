...
export const floatingApi = {
  ...
  createRegister: (data: any, config?: any) => request.post('/fp/register', data, config),
  updateRegister: (id: number, data: any) => request.put(`/fp/register/${id}`, data),
  deleteRegister: (id: number) => request.delete(`/fp/register/${id}`),
  // 居住地
  listResidence: (params?: any, config?: any) => request.get('/fp/residence', { params, ...config }),
  ...
  // 居住证
  listPermit: (params?: any, config?: any) => request.get('/fp/permit', { params, ...config }),
