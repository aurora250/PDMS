...
COMMENT ON COLUMN resident.nation_code IS '民族代码 - 符合 GB 3304-1991，01-56为标准民族，97=其他，98=外籍';
COMMENT ON COLUMN resident.education_code IS '学历代码 - 符合 GB/T 4658-2006';
...
CREATE TABLE IF NOT EXISTS household_business_request (
    ...
    business_type VARCHAR(20) NOT NULL CHECK (business_type IN ('出生登记','死亡注销','户口迁移','登记项目变更','分户立户')),
...
('民警组', '户籍民警：除日志审计和系统管理外的全部模块与操作',
 '["statistics:read",
   ...
   "fp:read","fp:write","fp:delete","fp:residence:write","fp:permit:approve","fp:permit:issue",
...
('采集员组', '一线采集员：常住人口（仅详情）、户籍管理（户口簿全操作+附加材料）、流动人口（仅居住地查看）、重点人口（仅列表）、失踪人口（仅详情）',
 '["resident:read",
   "household:read","household:write","household:material:attach",
   "fp:read",
   "keyperson:read",
   "missing:read"]'),
('街道办组', '街道办：采集员基础上 + 预警查看',
 '["resident:read",
   "household:read","household:write","household:material:attach",
   "fp:read",
   "keyperson:read",
   "missing:read",
   "alert:read"]'),
('数据审查员组', '独立数据审查：仅可见日志审计',
 '["log:audit:read","log:login:read"]'),
('市局负责人组', '市局：民警基础上 + 系统管理民警管理 + 特殊事项二审',
 '["statistics:read",
   "resident:read","resident:write","resident:import","resident:export","resident:change-request:second-approve",
   "household:read","household:write","household:approve","household:second-approve",
   "fp:read","fp:write","fp:delete","fp:residence:write","fp:permit:approve","fp:permit:issue",
   "keyperson:read","keyperson:write","keyperson:gis:read","keyperson:visit-plan:write","keyperson:petition:write",
   "missing:read","missing:write","missing:recovery:write",
   "alert:read","alert:handle",
