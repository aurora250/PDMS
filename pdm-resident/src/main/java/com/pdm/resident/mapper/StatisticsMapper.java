...
public interface StatisticsMapper {
    ...
    /** 户籍迁移流向数据（省份间流动统计，支持2/3级链路） */
    @Select("SELECT " + "  COALESCE(p_from.area_name, c_from.area_name, '未知') AS from_name, "
            + "  COALESCE(p_to.area_name, c_to.area_name, '未知') AS to_name, " + "  COUNT(*) AS value "
