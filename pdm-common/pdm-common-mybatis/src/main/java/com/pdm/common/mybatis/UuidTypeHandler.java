 ...
 *
 * <p>
 * 用于 MyBatis 在读写数据库 {@code uuid} 类型字段时进行自动转换：
 * <ul>
 * <li>写入时将 {@code String} 转为 {@code UUID} 对象存入 PostgreSQL</li>
 * <li>读取时将 PostgreSQL 返回的 {@code UUID} 对象转为 {@code String}</li>
 * <li>空值或空字符串写入时设为 SQL NULL</li>
 * </ul>
 */
