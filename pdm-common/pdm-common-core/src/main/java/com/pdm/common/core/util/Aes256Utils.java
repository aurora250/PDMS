 ...
 * AES-256-GCM 加密工具类。
 *
 * <p>用于敏感字段（如身份证号、手机号、密码）的加密存储和解密读取。 采用 GCM 模式提供认证加密，每次加密随机生成 12 字节 IV 并前置拼接在密文中。
...
public final class Aes256Utils {
     ...
     * 使用 AES-256-GCM 加密明文。
     *
     * <p>输出格式：Base64(IV + 密文)，IV 为 12 字节随机数。
     *
     * @param plainText 明文
     * @param base64Key Base64 编码的 256 位密钥
     ...
     * 使用 AES-256-GCM 解密密文。
     *
     * @param encryptedText Base64 编码的密文（含 IV 前缀）
     * @param base64Key Base64 编码的 256 位密钥
     * @return 解密后的明文字符串
     * @throws Exception 解密失败（密钥不匹配或数据损坏）时抛出
     */
