 ...
 *
 * <p>
 * 负责 JWT 访问令牌和刷新令牌的生成、解析、验证和信息提取。 使用 HMAC-SHA256 签名算法，密钥由 Base64 编码的密钥字符串初始化。
 * 访问令牌包含用户 UUID、用户名、角色和令牌类型等声明（claims）。
 * </p>
 */
@Slf4j
public class JwtTokenProvider {
     ...
     *
     * <p>
     * 令牌中包含用户 UUID（作为 subject）、用户名、角色和类型声明， 过期时间由 {@link #expirationMs} 指定。
     * </p>
     *
     * @param userUuid
     *            用户 UUID
     ...
     *
     * <p>
     * 刷新令牌有效期为访问令牌的 12 倍（默认 24 小时），仅包含类型声明和 subject， 不含用户详细信息，安全性更高。
     * </p>
     *
     * @param userUuid
     *            用户 UUID
