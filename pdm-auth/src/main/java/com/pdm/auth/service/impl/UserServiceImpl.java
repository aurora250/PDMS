 ...
 *
 * <ul>
 * <li><b>分页查询</b> —— 支持按用户名/UUID 关键词搜索，按角色和状态精确筛选。</li>
 * <li><b>按 UUID 查询</b> —— 根据用户唯一标识精确查找。</li>
 * <li><b>更新用户信息</b> —— 支持部分字段更新（手机、邮箱、角色、权限组）。</li>
 * <li><b>更新账号状态</b> —— 修改账号状态（审批中/有效/冻结/注销/锁定），锁定时自动设置 24 小时锁定。</li>
 * <li><b>删除用户</b> —— 仅允许删除非"有效"状态的用户，活跃用户需先停用。</li>
 * </ul>
 *
 * @author freedom
 */
...
public class UserServiceImpl implements UserService {
     ...
     * 支持以下筛选条件（均为可选）：
     * <ul>
     * <li>{@code keyword} —— 模糊匹配用户名和用户 UUID（OR 逻辑）</li>
     * <li>{@code role} —— 精确匹配用户角色</li>
     * <li>{@code status} —— 精确匹配账号状态</li>
     * </ul>
     * 结果按创建时间倒序排列。
     *
     * @param page
     ...
     *
     * <p>
     * 根据 UUID 定位用户，仅更新传入对象中非空的字段：
     * <ul>
     * <li>手机号（{@code phone}）</li>
     * <li>邮箱（{@code email}）</li>
     * <li>角色（{@code userRole}）</li>
     * <li>权限组 ID（{@code permissionGroupId}）</li>
     * </ul>
     *
     * @param userUuid
     *            用户唯一标识
     * @param updates
     *            包含待更新字段的用户对象（仅非空字段生效）
     * @return 更新后的用户实体
     * @throws BusinessException
     *             用户不存在时抛出
     */
     ...
     * 仅允许以下五种状态值：
     * <ul>
     * <li>{@code "审批中"} —— 账号待审批</li>
     * <li>{@code "有效"} —— 账号正常可用</li>
     * <li>{@code "冻结"} —— 账号已冻结</li>
     * <li>{@code "注销"} —— 账号已注销</li>
     * <li>{@code "锁定"} —— 账号锁定（自动设置 24 小时后解锁）</li>
     * </ul>
     *
     * @param userUuid
     *            用户唯一标识
     ...
     *
     * <p>
     * 仅允许删除非"有效"状态的用户，活跃用户需先调用 {@link #updateUserStatus(String, String)} 停用后再删除。
     *
     * @param userUuid
     *            用户唯一标识
     * @throws BusinessException
