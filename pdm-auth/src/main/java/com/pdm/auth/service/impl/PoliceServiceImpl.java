 ...
 *
 * <ul>
 *   <li><b>注册警员</b> —— 新增警员记录，未指定警号时自动生成唯一警号。
 *   <li><b>分页查询</b> —— 支持按警号、派出所、部门进行关键词模糊搜索，按创建时间倒序排列。
 *   <li><b>按警号查询</b> —— 根据警号精确查找警员信息。
 *   <li><b>更新警员信息</b> —— 支持部分字段更新（派出所、辖区、区域、部门、警衔）。
 *   <li><b>更新值班状态</b> —— 修改警员的在岗状态，仅允许"在岗""调岗""离职"三种值。
 * </ul>
 *
 * @author freedom
 */
...
public class PoliceServiceImpl implements PoliceService {
     ...
     * 注册警员。
     *
     * <p>若传入的警员未指定警号，则自动以 {@code "P"} 为前缀生成 15 位唯一标识作为警号。 生成后会校验警号是否已存在，重复则抛出异常。
     *
     * @param police 待注册的警员实体
     * @return 注册成功后的警员实体（含自动生成的警号）
     * @throws BusinessException 警号已存在时抛出
     */
    @Override
    @Transactional
     ...
     * 分页查询警员列表。
     *
     * <p>支持按关键词模糊搜索警号、派出所和部门（多字段 OR 匹配），结果按创建时间倒序排列。 关键词为空时返回全部记录。
     *
     * @param page 页码（从 1 开始）
     * @param size 每页条数
     * @param keyword 搜索关键词（可选，匹配警号、派出所、部门）
     ...
     * @return 警员实体
     * @throws BusinessException 警员不存在时抛出
     */
    @Override
     ...
     * 更新警员信息。
     *
     * <p>根据警号定位警员，仅更新传入对象中非空的字段：
     *
     * <ul>
     *   <li>派出所（{@code policeStation}）
     *   <li>辖区（{@code jurisdiction}）
     *   <li>区域 ID（{@code areaId}）
     *   <li>部门（{@code department}）
     *   <li>警衔（{@code policeRank}）
     * </ul>
     *
     * @param policeNumber 警号
     * @param updates 包含待更新字段的警员对象（仅非空字段生效）
     * @return 更新后的警员实体
     * @throws BusinessException 警员不存在时抛出
     */
    @Override
     ...
     *
     * @param policeNumber 警号
     * @param dutyStatus 值班状态（必须为"在岗""调岗""离职"之一）
     * @throws BusinessException 警员不存在或状态值无效时抛出
     */
    @Override
    @Transactional
