 ...
 * 用户管理控制器。
 *
 * <p>提供用户的查询、更新、状态管理和删除 REST API 接口，挂载在 {@code /api/auth/users} 路径下。 需要系统管理员或用户管理员角色方可访问。
 */
@RestController
...
public class UserController {
     ...
     * 分页查询用户列表。
     *
     * <p>支持按关键词（用户名/UUID）、角色和状态进行多条件筛选。
     *
     * @param page 页码，默认 1
     * @param size 每页条数，默认 20
     * @param keyword 搜索关键词（可选，模糊匹配用户名或 UUID）
     * @param role 角色筛选（可选）
    ...
    public Result<PageResult<User>> listUsers(
        ...
        return Result.success(
                PageResult.of(userPage.getRecords(), userPage.getTotal(), page, size));
    }
     ...
     * 更新用户账号状态。
     *
     * <p>支持设置为"审批中""有效""冻结""注销""锁定"五种状态， 设为"锁定"时自动设置 24 小时后解锁。
     *
     * @param uuid 用户唯一标识
     * @param body 请求体，包含 {@code status} 字段
     * @return 操作结果
     */
     ...
     * 删除用户。
     *
     * <p>仅允许删除非"有效"状态的用户，活跃用户需先停用后再删除。
     *
     * @param uuid 用户唯一标识
     * @return 操作结果
     */
